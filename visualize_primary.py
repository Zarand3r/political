...
def load_data(filename):
	ratings = pd.read_csv(filename, encoding='latin1', names=['year', 'FIPS', 'candidate', 'votes'])
	df = pd.DataFrame(ratings)
	df = df.dropna()
	# make a nnew column as dataframe["votes"] with ratio of candidatevotes/totalvotes
	df = df[df['candidate'] != "Other (+)"]
	df = df[df['candidate'] != "Uncommitted"]
	df = df[df['votes'] > 10]
	df['FIPS'] = df['FIPS'].astype(int)
	df['candidate'] = df['year'].map(str) + "_" + df['candidate']
	df['votes'] = df['votes']/100
...
def trainSVD(X, colorlabels, plot=True, savefig="figures/"):
	...
	if plot:
		...
		if savefig:
			plt.savefig(savefig+"algebraic_svd_counties")
		plt.show()


def trainSVD_surprise(training_data, colorlabels, plot=True, simplify=False, savefig="figures/"): #colorlabels, sizelabels, plot=True, savefig=True
...
def trainSVD_surprise3D(training_data, colorlabels, plot=True, savefig="figures/"): #colorlabels, sizelabels, plot=True, savefig=True
	# algo = SVD(n_factors=4, n_epochs=1000, biased=True)
	# algo = SVD(n_factors=20, n_epochs=500, biased=False)
...
def visualize(filename, savefig="figures/"):
	...
	trainSVD_surprise(ts, states_labels, savefig=savefig)
...
if __name__ == '__main__':
	filename = 'data/primary_democratic.csv'
	visualize(filename, savefig="figures/primary_democratic/")
